...
class UConfigBPPluginBPLibrary : public UBlueprintFunctionLibrary
{
	...
	*/
	 ...
	 * Returns all of the keys in a given configuration file's section
	 *
	 * @param File The configuration file the key is in
	 * @param Section The section the key is in
	 * @param Key The key that you'd like to remove
	 *
	 * @return Whether or not the removal was a success
	 */
	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Remove Configuration Key", Keywords = "ConfigBPPlugin"), Category = "ConfigBP")
	static void RemoveKey(UConfigFile* File, FString Section, FString Key, bool& bSuccess);
};