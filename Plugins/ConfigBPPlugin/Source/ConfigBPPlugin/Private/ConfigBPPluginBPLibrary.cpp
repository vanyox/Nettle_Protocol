...
void UConfigBPPluginBPLibrary::RemoveKey(UConfigFile* File, FString Section, FString Key, bool& bSuccess)
{
	if (!IsValid(File) || !GConfig || Section.IsEmpty() || Key.IsEmpty())
	{
		bSuccess = false;
		return;
	}

	bSuccess = GConfig->RemoveKey(*Section, *Key, File->GetFilePath());
	GConfig->Flush(false, File->GetFilePath());
	// NOTE: This change won't automatically be saved for the file; SaveCurrentStateForBootstrap was tried but turned out to be unsuccessful.
	return;
}
